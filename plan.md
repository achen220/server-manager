...
- [ ] **Real SSH terminal** — add `xterm.js`, open WebSocket/Socket.io channel in NestJS, pipe `ssh2` shell stream ↔ WebSocket; register `TerminalComponent` in router at `/connections/:id/terminal`
- [ ] **Add SSH user dialog** — fill in empty `p-dialog` in `ServerManagerComponent` with reactive form; add missing `addSshUser()` HTTP call in frontend `ServerService`
- [ ] **Fix `ActiveSSHInfo` interface** — align frontend type with backend response `{ username, uid, gid, home, shell, isActive, lastLogin, isAdmin, hasValidShell }`
- [ ] **`GET /api/connections/:id`** — add missing `@Get(':id')` controller route (service method already exists)
- [ ] **SSH key content storage** — store key content encrypted via `EncryptionService` instead of a local filesystem path
